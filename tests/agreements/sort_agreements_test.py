...
async def test_sort_agreements_by_deadline_asc(
    ...
    assert (
        agreements[0]["deadline"] == date(2024,1, 1).isoformat()
    )
    assert (
        agreements[1]["deadline"] == date(2024,2, 2).isoformat()
    )
    assert (
        agreements[2]["deadline"] == date(2025,3, 3).isoformat()
    )
...
async def test_sort_agreements_by_deadline_desc(
    ...
    assert (
        agreements[0]["deadline"] == date(2025,3, 3).isoformat()
    )
    assert (
        agreements[1]["deadline"] == date(2024,2, 2).isoformat()
    )
    assert (
        agreements[2]["deadline"] == date(2024,1, 1).isoformat()
    )
...
async def test_sort_agreements_by_number_asc(
    ...
    """
    response = client.get(
        "/agreements", params={"sortBy": "number", "sortDir": "asc"}
    )
    assert response.status_code == 200

    agreements = response.json()
    assert (
        agreements[0]["number"] == "11111"
    )
    assert (
        agreements[1]["number"] == "1234567890"
    )
    assert (
        agreements[2]["number"] == "987654321"
    )
...
async def test_sort_agreements_by_number_desc(
    ...
    """
    response = client.get(
        "/agreements", params={"sortBy": "number", "sortDir": "desc"}
    )
    assert response.status_code == 200

    agreements = response.json()
    assert (
        agreements[0]["number"] == "987654321"
    )
    assert (
        agreements[1]["number"] == "1234567890"
    )
    assert (
        agreements[2]["number"] == "11111"
    )
...
async def test_sort_agreements_by_notes_asc(
    ...
    """
    response = client.get(
        "/agreements", params={"sortBy": "notes", "sortDir": "asc"}
    )
    assert response.status_code == 200

    agreements = response.json()
    assert (
        agreements[0]["notes"] == "first notes"
    )
    assert (
        agreements[1]["notes"] == "new_test notes"
    )
    assert (
        agreements[2]["notes"] == "third notes"
    )
...
async def test_sort_agreements_by_notes_desc(
    ...
    """
    response = client.get(
        "/agreements", params={"sortBy": "notes", "sortDir": "desc"}
    )
    assert response.status_code == 200

    agreements = response.json()
    assert (
        agreements[0]["notes"] == "third notes"
    )
    assert (
        agreements[1]["notes"] == "new_test notes"
    )
    assert (
        agreements[2]["notes"] == "first notes"
    )
...
async def test_sort_agreements_by_contract_asc(
    ...
    agreements = response.json()
    assert (
        agreements[0]["contract"]["name"] == "fourth_contract_name"
    )
    assert (
        agreements[1]["contract"]["name"] == "new_test_name_1"
    )
    assert (
        agreements[2]["contract"]["name"] == "test_name_2"
    )
...
async def test_sort_agreements_by_contract_desc(
    ...
    agreements = response.json()
    assert (
            agreements[0]["contract"]["name"] == "test_name_2"
    )
    assert (
            agreements[1]["contract"]["name"] == "new_test_name_1"
    )
    assert (
            agreements[2]["contract"]["name"] == "fourth_contract_name"
    )