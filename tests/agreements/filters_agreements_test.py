def test_filters_agreements(
    ...
    response = client.get(
        "/agreements", params={"name": [another_agreement.name, sample_agreement.name]}
    )
    assert response.status_code == 200
    assert response.json() == [
        {
    ...
    response = client.get(
        "/agreements",
        params={"price": [another_agreement.price, sample_agreement.price]},
    )
    assert response.status_code == 200
    assert response.json() == [
        {
    ...
    response = client.get(
        "/agreements",
        params={"contract_name": [sample_contract.name, another_contract.name]},
    )
    assert response.status_code == 200
    assert response.json() == [
        {
