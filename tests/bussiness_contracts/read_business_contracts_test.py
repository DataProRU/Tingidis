...
def test_get_agreements(client, sample_business_contract, another_business_contract):
    ...
    contracts = sorted(
        [sample_business_contract, another_business_contract], key=lambda x: x.code
    )
    ...
    expected_result = [
        {
            ...
            "price": (
                float(contract.price)
                if isinstance(contract.price, Decimal)
                else contract.price
            ),
    ...
    expected_response = [
        {
            ...
            "price": (
                float(sample_business_contract.price)
                if isinstance(sample_business_contract.price, Decimal)
                else sample_business_contract.price
            ),
