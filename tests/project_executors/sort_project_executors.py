...
async def test_sort_project_executors_by_user_full_name_asc(
    ...
    """
    response = client.get(
        "/project-executors", params={"sortBy": "user_info.full_name", "sortDir": "asc"}
    )
    assert response.status_code == 200

    executors = response.json()
    assert len(executors) >= 3
    assert executors[0]["user"]["full_name"] == "Ivan Ivanov"
    assert executors[1]["user"]["full_name"] == "Alex Ivanovich"
...
async def test_sort_project_executors_by_user_full_name_desc(
    ...
    response = client.get(
        "/project-executors",
        params={"sortBy": "user_info.full_name", "sortDir": "desc"},
    )
    assert response.status_code == 200
    ...
    assert executors[1]["user"]["full_name"] == "Alex Ivanovich"
    assert executors[2]["user"]["full_name"] == "Ivan Ivanov"
...
async def test_sort_project_executors_by_project_name_asc(
    ...
    """
    response = client.get(
        "/project-executors", params={"sortBy": "project_info.name", "sortDir": "asc"}
    )
    assert response.status_code == 200

    executors = response.json()
    assert len(executors) >= 3
    assert executors[0]["project"]["name"] == "Sample Project"
    assert executors[1]["project"]["name"] == "Another Project"
...
async def test_sort_project_executors_by_project_name_desc(
    ...
    """
    response = client.get(
        "/project-executors", params={"sortBy": "project_info.name", "sortDir": "desc"}
    )
    assert response.status_code == 200
    ...
    assert executors[1]["project"]["name"] == "Another Project"
    assert executors[2]["project"]["name"] == "Sample Project"
